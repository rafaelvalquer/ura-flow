...
export async function parseExcelFileInWorker(file, onProgress = () => {}) {
  ...
  return parseExcelBuffer(file.name, buffer, onProgress);
}

export async function parseExcelBuffer(fileName, buffer, onProgress = () => {}) {
  onProgress({
    percent: 8,
  ...
  for (const [index, sheetName] of sheetNames.entries()) {
    ...
    if (!table) {
      warnings.push({
        ...
        severity: 'warning',
        sheetName,
        message: `A aba "${sheetName}" nao possui tabela de Resultado.`,
      });
      continue;
    }
...
function parseBranch(text) {
  ...
  const suffix = cleaned.match(/^(.+?)\s+(SIM|NAO|ligada|desligada|ativo|ativa|desativado|desativada|adimplente|inadimplente)$/i);
  if (suffix) {
  ...
  nodes.forEach((node) => {
    if (!node.destination && !node.isDirective && node.children.length === 0) {
      warnings.push({
        ...
        message: `Linha ${node.rowNumber} possui condicao sem destino final.`,
      });
    }
  });
}
  ...
  allTransitions.forEach((transition) => {
    ...
    if (type === 'unknown') {
      ...
      warnings.push({
        ...
        message: `Destino "${transition.to}" nao encontrado como aba.`,
      });
    }
  });
